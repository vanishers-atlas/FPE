...
def test_layer(input_neurons, output_neurons, packing_factor, number_tests, test_time):
    parameters = generate_parameters(input_neurons, output_neurons)

    # Generate ROMs
    rom_A = []
    rom_A_width = max(tc_utils.unsigned.width(input_neurons), packing_factor)

    for output in range(output_neurons):
        # Add weights for this output neuron
        for weight in range(input_neurons):
            if weight % packing_factor == 0:
                acc = parameters[output]["weights"][weight]
            else:
                acc *= 2
                acc += parameters[output]["weights"][weight]
                if (weight + 1) % packing_factor == 0:
                    rom_A.append(acc)
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", input_neurons, output_neurons, packing_factor, use_BRAMs=False, pregened_memfiles=["ROM_A"] )
...
if __name__ == "__main__":
    ...
    print("verifying packing_factor factor = input_neurons")
    test_layer(input_neurons, output_neurons, input_neurons, number_tests, test_time)
    print()

    print("verifying packing_factor factor = factor of input_neurons")
