...
def generate_layer_fpea(path, layer_name, input_rows, input_cols, data_depth, packing_factor):
    ...
    fpea += "\<}\n"
    fpea += "MOV (ACC, RAM[BAM[first_element]<FORWARD>]);\n"
    fpea += "\<}\n"
    fpea += "\n"
    ...
    fpea += "ADD (GET[0]<ADV>, ACC, ACC);\n"
    fpea += "\<}\n"
    fpea += "OR (ACC, RAM[BAM[second_element]<FORWARD>], RAM[BAM[second_element]<FORWARD>]);\n"
    fpea += "\<}\n"
    fpea += "NOP;\n"
    fpea += "\n"
    ...
    fpea += "\<}\n"
    fpea += "\n"
    fpea += "OR (ACC, RAM[BAM[first_element]<FORWARD>], RAM[BAM[first_element]<FORWARD>]);\n"
    fpea += "\<}\n"
    fpea += "\n"
    ...
    fpea += "ADD (GET[0]<ADV>, ACC, ACC);\n"
    fpea += "\<}\n"
    fpea += "OR (ACC, RAM[BAM[second_element]<FORWARD>], ACC);\n"
    fpea += "ZOL (packing_factor)\n"
    fpea += "{\>\n"
    fpea += "LRL (ACC, 1, PUT[0]);\n"
    fpea += "\n"
    fpea += "\<}\n"
    fpea += "NOP;\n"
    fpea += "\<}\n"
