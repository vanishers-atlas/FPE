...
def generate_layer(path, layer_name, data_cols, data_rows, data_depth, num_kernals, packing_factor, use_BRAMs=False, pregened_memfiles=None):
    # Generate toolchain input files
    generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals, packing_factor)
    parameters, generics = generate_layer_parameters_and_generics(path, layer_name, data_cols, data_rows, data_depth, num_kernals, packing_factor, use_BRAMs)
...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals, packing_factor):
    ...
    fpea += "DEF packed_depth data_depth/packing_factor ;\n"
    fpea += "\n"
    ...
    fpea += "// Set packing,\n"
    assert packing_factor % 2 == 0
    fpea += "MOV(0b10, ACC);\n"
    fpea += "ZOL (packing_factor/2)\n"
    fpea += "{\>\n"
    fpea += "LSH(ACC, 2, ACC);\n"
    ...
    assert data_depth % packing_factor == 0
    fpea += "ZOL (data_cols * packed_depth)\n"
    fpea += "{\>\n"
    ...
    fpea += "SEEK BAM[act_row_0] (3 * packed_depth)<BACKWARD>;\n"
    fpea += "SEEK BAM[act_row_1] (3 * packed_depth)<BACKWARD>;\n"
    fpea += "SEEK BAM[act_row_2] (3 * packed_depth)<BACKWARD>;\n"

    fpea += "\<}\n"
    fpea += "\n"

    fpea += "// Seek acc_row BAMs forward for next col\n"
    fpea += "SEEK BAM[act_row_0] (packed_depth)<FORWARD>;\n"
    fpea += "SEEK BAM[act_row_1] (packed_depth)<FORWARD>;\n"
    fpea += "SEEK BAM[act_row_2] (packed_depth)<FORWARD>;\n"

    fpea += "\<}\n"
    fpea += "\n"

    fpea += "// Seek acc_row BAMs forward for next row\n"
    fpea += "SEEK BAM[act_row_0] (2 * packed_depth)<FORWARD>;\n"
    fpea += "SEEK BAM[act_row_1] (2 * packed_depth)<FORWARD>;\n"
    fpea += "SEEK BAM[act_row_2] (2 * packed_depth)<FORWARD>;\n"
...
def generate_layer_parameters_and_generics(path, layer_name, data_cols, data_rows, data_depth, num_kernals, packing_factor, use_BRAMs):
    assert data_depth % packing_factor == 0
    packed_depth = int(data_depth/packing_factor)

    kernal_ROM_depth = num_kernals * 3 * 3 * packed_depth
    ...
    RAM_depth = 2 ** math.ceil(math.log((3 * (2 + data_cols) * packed_depth), 2))

    popcount_width = tc_utils.unsigned.width(3 * 3 * data_depth)
    packing_width = max(popcount_width, packing_factor)
    ALU_width = max(popcount_width + 1, packing_factor)
    ...
    parameters = {
    	...
    	"address_sources": {
    		...
    		"BAM_1": {
    			"addr_max": RAM_depth - 1,
    			"offset_max": RAM_depth - 1,
    			"step_max": 3 * packed_depth * (data_cols + 2)
    		},
            # act_row_1 BAM
    		"BAM_2": {
    			"addr_max": RAM_depth - 1,
    ...
    generics = {
    	"BAM_0_base": 0,
    	"BAM_0_increment": 1,
    	"BAM_1_base": 0*(data_cols + 2)*packed_depth,
    	"BAM_1_increment": 1,
        "BAM_2_base": 1*(data_cols + 2)*packed_depth,
    	"BAM_2_increment": 1,
        "BAM_3_base": 2*(data_cols + 2)*packed_depth,
    	"BAM_3_increment": 1,
    	"BAM_4_base": 0,
    	"BAM_4_increment": 1,
