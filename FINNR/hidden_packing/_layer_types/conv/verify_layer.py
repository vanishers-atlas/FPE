...
def test_layer(data_rows, data_cols, data_depth, num_kernals, packing_factor, number_tests, test_time):
    # Generate kernals
    ...
    # Generate ROMs01
    rom_A = []
    assert packing_factor <= data_depth
    assert data_depth % packing_factor == 0
    num_padding = int(data_depth/packing_factor)

    popcount_width = tc_utils.unsigned.width(9 * data_depth)
    rom_A_width = max(popcount_width, packing_factor)
    import json
    with open("kernal.json", "w") as f:
        f.write(json.dumps(kernals, indent=2))
    # Handle kernal region
    for k in range(num_kernals):
        for c in [-1, 0, 1]:
            for p in range(0, num_padding):
                k0 = kernals[k][-1][c][packing_factor*p]
                k1 = kernals[k][ 0][c][packing_factor*p]
                k2 = kernals[k][ 1][c][packing_factor*p]
                for b in range(1, packing_factor):
                    k0 *= 2
                    k0 += kernals[k][-1][c][packing_factor*p + b]
                    k1 *= 2
                    k1 += kernals[k][ 0][c][packing_factor*p + b]
                    k2 *= 2
                    k2 += kernals[k][ 1][c][packing_factor*p + b]
                rom_A.append(k0)
                rom_A.append(k1)
                rom_A.append(k2)
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", data_cols, data_rows, data_depth, num_kernals, packing_factor, use_BRAMs=False, pregened_memfiles=["ROM_A", ] )
...
if __name__ == "__main__":
    ...
    print("verifying packing_factor factor = data_depth")
    test_layer(data_rows, data_cols, data_depth, num_kernals, data_depth, number_tests, test_time)
    print()

    print("verifying packing_factor factor = factor of data_depth")
    test_layer(data_rows, data_cols, data_depth, num_kernals, int(data_depth/3), number_tests, test_time)
    print()
