...
def test_layer(data_rows, data_cols, data_depth, num_kernals, num_lanes, number_tests, test_time):
    assert num_kernals % num_lanes == 0
    kernals_per_lane = math.floor(num_kernals/num_lanes)

    # Generate kernals
    kernals = []
    for lane in range(num_lanes):
        kernals.append(generate_kernals(kernals_per_lane, data_depth))

    # Generate parameters
    parameters = []
    for lane in range(num_lanes):
        parameters.append(generate_parameters(kernals_per_lane, data_rows, data_cols, data_depth))
    ...
    for lane in range(num_lanes):
        ...
        for k in range(kernals_per_lane):
            for c in [-1, 0, 1]:
                for d in range(data_depth):
                    for r in [-1, 0, 1]:
                        rom_A.append(kernals[lane][k][r][c][d])

        # Handle parameters region
        for r in range(data_rows):
            for c in range(data_cols):
                for k in range(kernals_per_lane):
                    rom_A.append(parameters[lane][r][c][k]["threashold"])
                    # Gammes are inverted to go from math sign function to 2 comp sign style
                    rom_A.append(parameters[lane][r][c][k]["gamma"])
    ...
    test_output = []
    for lane in range(num_lanes):
        test_output.append(generate_data_output(test_input, kernals[lane], parameters[lane]))
