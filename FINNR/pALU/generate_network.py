...
# input layer types
from _layer_types import conv
from _layer_types import pool
from _layer_types import dense
from _layer_types import acc
...
def generation_network(network_name, input_dims, layers, use_BRAMs=True):
    ...
    for layer, layer_details in enumerate(layers):
        ...
        if   layer_details["type"] == conv:
            if layer_details["packing_factor"] == None:
                layer_details["packing_factor"] = conv.compute_parallelism_factor(layer_width, layer_height, layer_depth)
            ...
            if layer_details["packing_factor"] == None:
                layer_details["packing_factor"] = pool.compute_parallelism_factor(layer_width, layer_height, layer_depth)
            ...
            if layer_details["packing_factor"] == None:
                layer_details["packing_factor"] = dense.compute_parallelism_factor(input_neurons)
            ...
            input_neurons = layer_width * layer_height * layer_depth
            next_data_width = acc.compute_popcount(input_neurons)
            if layer_details["packing_factor"] == None:
                layer_details["packing_factor"] = acc.compute_parallelism_factor(input_neurons)
