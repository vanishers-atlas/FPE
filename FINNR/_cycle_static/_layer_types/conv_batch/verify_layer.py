...
def test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time, batch_size):
    # Generate kernals
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", data_cols, data_rows, data_depth, num_kernals, batch_size, use_BRAMs=False, pregened_memfiles=["ROM_A", ] )
    ...
    assert data_depth % batch_size == 0, "batch_size must be a factor of data_depth for pool_batch layer type"
    generate_testbrench_file(test_input, test_output, number_tests, data_rows*data_cols*int(data_depth/batch_size))
...
if __name__ == "__main__":
    ...
    print("Testing layer, batch size data_depth")
    test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time, data_depth)

    print("Testing layer, batch size 3")
    test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time, 3)

    print("Testing layer, batch size 1")
    test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time, 1)
