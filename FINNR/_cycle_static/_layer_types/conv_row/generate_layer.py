...
def compute_kickoff_data(data_cols, data_rows, data_depth, num_kernals):
    return data_cols * data_depth

def compute_kickoff_space(data_cols, data_rows, data_depth, num_kernals):
    return data_cols * num_kernals
...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals):
    ...
    fpea += "DEF popcount 1 ;\n"
    fpea += "DEF rowcount 2 ;\n"
    fpea += "DEF frame_started  3 ;\n"
    fpea += "\n"


    fpea += "// Fan out based on rowcount\n"
    fpea += "UCMP (REG[frame_started], 1);\n"
    fpea += "UCMP (REG[rowcount], data_rows);\n"
    fpea += "JNE (PRE_FRAME);\n"
    fpea += "JEQ (PAD_ROW);\n"
    fpea += "NOP;\n"
    fpea += "NOP;\n"
    ...
    fpea += "READ_ROW :\n"
    fpea += "ADD (REG[rowcount], 1, REG[rowcount]);\n"
    ...
    fpea += "NOT(ACC, RAM[BAM[act_write]<FORWARD>] ) ;\n"
    fpea += "\<}\n"
    fpea += "NOT(ACC, REG[packing]) ;\n"
    fpea += "\<}\n"
    fpea += "\n"
    fpea += "\n"


    fpea += "UCMP (REG[rowcount], 0);\n"
    fpea += "NOP;\n"
    fpea += "JEQ (READ_ROW);\n"
    fpea += "JNE (PROCESS_ROW);\n"
    fpea += "NOP;\n"
    fpea += "NOP;\n"
    fpea += "NOP;\n"
    fpea += "\n"
...
def generate_layer_parameters_and_generics(path, layer_name, data_cols, data_rows, data_depth, num_kernals, use_BRAMs):
    ...
    rowcount_width = tc_utils.unsigned.width(data_cols)
    parameters = {
    	...
    	"data_memories": {
    		...
    		"REG": {
    			"data_width": max(popcount_width, rowcount_width),
    			"depth": 4
    		},
    		"ROM_A": {
