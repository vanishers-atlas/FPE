...
def compute_kickoff_data(input_neurons, output_neurons, batch_size):
	return batch_size

def compute_kickoff_space(input_neurons, output_neurons, batch_size):
	return output_neurons

def generate_layer(path, layer_name, input_neurons, output_neurons, batch_size, use_BRAMs=False, pregened_memfiles=None):
	# Generate toolchain input files
	generate_layer_fpea(path, layer_name, input_neurons, output_neurons, batch_size)
...
def generate_layer_fpea(path, layer_name, input_neurons, output_neurons, batch_size):
	fpea = IndentedString()

	assert type(batch_size) == int and batch_size > 0, "batch_size must is a position int"
	...
	fpea += "NOP ;\n"
	fpea += "JEQ (READ_DATA_START) ;\n"
	fpea += "NOP ;\n"
	fpea += "NOP ;\n"
	fpea += "NOP ;\n"
	fpea += "\n"

	fpea += "// RESET BAMs for new frame if framed has been started \n"
	fpea += "JMP (READ_DATA_START) ; // Jump instr ahead of reset and flaging at fill jump shadow\n"
	fpea += "RESET BAM[acts] ;\n"
	fpea += "RESET BAM[weights] ;\n"
	fpea += "MOV (1, REG[frame_started]) ;\n"
	fpea += "\n"

	if batch_size == 1:
		fpea += "// Handle data input\n"
		...
		fpea += "READ_DATA_START :\n"
		fpea += "MOV (GET[0]<ADV>, RAM[BAM[acts]<FORWARD>] ) ;\n"
		fpea += "\<}\n"
		fpea += "\n"
	else:
		assert input_neurons % batch_size == 0, "batch size must be a factor of input_neurons"
		fpea += "// Handle data input\n"
		fpea += "ZOL (input_neurons/batch_size)\n"
		fpea += "{\>\n"
