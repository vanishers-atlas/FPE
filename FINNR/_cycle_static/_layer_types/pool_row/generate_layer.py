...
def generate_layer_fpea(path, layer_name, input_rows, input_cols, data_depth):
    ...
    fpea += "DEF row_parity_loc   0 ;\n"
    fpea += "DEF row_parity_upper 0 ;\n"
    fpea += "DEF row_parity_lower 1 ;\n"
    fpea += "\n"
    ...
    fpea += "UCMP (REG[row_parity_loc], row_parity_lower);\n"
    fpea += "NOP;\n"
    fpea += "JEQ (LOWER_ROW);"
    fpea += "RESET BAM[first_element];\n"
    fpea += "RESET BAM[second_element];\n"
    fpea += "// Omitted as the write to row_parity_loc will immedatally overwritten following the jump\n"
    fpea += "// NOP;\n"
    fpea += "\n"

    fpea += "// Handle upper row of pairs\n"
    fpea += "UPPER_ROW :\n"
    fpea += "MOV (row_parity_lower, REG[row_parity_loc]);  // Toggle row_parity\n"
    ...
    fpea += "// Jump to the end of the program to await next row's data\n"
    fpea += "JMP (END);"
    fpea += "NOP;\n"
    fpea += "NOP;\n"
    fpea += "NOP;\n"
    ...
    fpea += "LOWER_ROW :\n"
    fpea += "MOV (row_parity_upper, REG[row_parity_loc]);  // Toggle row_parity\n"
