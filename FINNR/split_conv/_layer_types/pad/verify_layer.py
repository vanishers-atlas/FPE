...
def generate_data_output(data_input):
    ...
    for test in range(len(data_input)):
        ...
        for r in range(data_rows + 2):
            test_output[test][r] = {}
            for c in range(data_cols + 2):
                test_output[test][r][c] = {}
                try:
                    for d in range(data_depth):
                        test_output[test][r][c][d] = data_input[test][r-1][c-1][d]
                except Exception as e:
                    for d in range(data_depth):
                        test_output[test][r][c][d] = (r + c + d + 3) % 2
...
def test_layer(data_rows, data_cols, data_depth, number_tests, test_time):
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", data_cols, data_rows, data_depth, use_BRAMs=False, pregened_memfiles=[] )
