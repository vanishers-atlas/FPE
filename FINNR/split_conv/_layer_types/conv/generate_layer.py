...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals):
    ...
    fpea += "// Handle first 2 rows row,\n"
    fpea += "// read of middle cols\n"
    fpea += "ZOL (2 * data_depth * data_cols)\n"
    fpea += "{\>\n"
    fpea += "MOV( GET[0]<ADV>, RAM[BAM[act_write]<FORWARD>] ) ;\n"
    fpea += "\<}\n"

    fpea += "// Process all rows\n"
    fpea += "ZOL (data_rows - 2)\n"
    fpea += "{\>\n"
...
def generate_layer_parameters_and_generics(path, layer_name, data_cols, data_rows, data_depth, num_kernals, use_BRAMs):
    ...
    # 3 for 3 rows
    # data_cols for padded rows
    # data_depth for data depth
    RAM_depth = 2 ** math.ceil(math.log((3 * data_cols * data_depth), 2))
    ...
    parameters = {
    	...
    	"address_sources": {
    		...
    		"BAM_1": {
    			"addr_max": RAM_depth - 1,
    			"offset_max": RAM_depth - 1,
    			"step_max": 3 * data_depth * data_cols
    		},
            # act_row_1 BAM
    		"BAM_2": {
    			"addr_max": RAM_depth - 1,
    ...
    generics = {
    	"BAM_0_base": 0,
    	"BAM_0_increment": 1,
    	"BAM_1_base": 0*data_cols*data_depth,
    	"BAM_1_increment": 1,
        "BAM_2_base": 1*data_cols*data_depth,
    	"BAM_2_increment": 1,
        "BAM_3_base": 2*data_cols*data_depth,
    	"BAM_3_increment": 1,
    	"BAM_4_base": 0,
    	"BAM_4_increment": 1,
