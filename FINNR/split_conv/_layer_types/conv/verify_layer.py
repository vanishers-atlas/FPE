...
def generate_data_output(data_input, kernals, parameters):
    ...
    test_output = {}
    #if True:
    with open("popcounts.txt", "w") as f:
            ...
            for row in range(data_rows - 2):
                ...
                for col in range(data_cols - 2):
                    ...
                    for kernal in range(len(kernals)):
                        assert sorted(kernals[kernal].keys()) == [0, 1, 2], "kernal with incorrect rows"
                        assert sorted(kernals[kernal][0].keys()) == [0, 1, 2], "kernal with incorrect cols"
                        assert sorted(kernals[kernal][1].keys()) == [0, 1, 2], "kernal with incorrect cols"
                        assert sorted(kernals[kernal][2].keys()) == [0, 1, 2], "kernal with incorrect cols"
                        ...
                        for kr in [0, 1, 2]:
                            for kc in [0, 1, 2]:
                                for d in range(data_depth):
                                    xnor = 1 if data_input[test][row + kr][col + kc][d] == kernals[kernal][kr][kc][d] else 0
                                    acc += xnor
...
def test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time):
    if True:
        # Generate kernals
        ...
        with open("ROM.txt", "w") as f:
            ...
            for r in range(data_rows):
                for c in range(data_cols):
                    for k in range(num_kernals):
                        f.write("%s %i\n"%(tc_utils.unsigned.encode(parameters[r][c][k]["threashold"], rom_A_width), parameters[r][c][k]["gamma"], ))


    # Generate program files
    generate_layer.generate_layer(".\\verifying", "varifying", data_cols, data_rows, data_depth, num_kernals, use_BRAMs=False, pregened_memfiles=["ROM_A", ] )
...
if __name__ == "__main__":
    ...
    test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time)
