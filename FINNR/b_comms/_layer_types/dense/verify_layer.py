...
def generate_testbrench_file(input_data, output_data):
    with open(".\\verifying\\testbench.vhd", "w") as f:
        ...
        f.write("\t\t\tGET_FIFO_0_valid  => GET_FIFO_0_valid,\n")
        f.write("\t\t\tPUT_FIFO_0_data  => PUT_FIFO_0_data,\n")
        f.write("\t\t\tPUT_FIFO_0_write => PUT_FIFO_0_write,\n")
        f.write("\t\t\tPUT_FIFO_0_ready => PUT_FIFO_0_ready,\n")
        f.write("\t\t\tclock => clock,\n")
        ...
        f.write("\tvalid_gen : process\n")
        f.write("\tbegin\n")
        f.write("\t\twait until running = '1';\n")
        f.write("\t\twait until rising_edge(clock);\n")
        f.write("\t\tPUT_FIFO_0_ready <= '0';\n")
        f.write("\t\tloop\n")
        f.write("\t\twait for 1000 ns;\n")
        f.write("\t\t\twait until rising_edge(clock);\n")
        f.write("\t\t\tPUT_FIFO_0_ready <= '1';\n")
        f.write("\t\t\twait until PUT_FIFO_0_write = '1';\n")
        f.write("\t\t\twait until rising_edge(clock);\n")
        f.write("\t\t\tPUT_FIFO_0_ready <= '0';\n")
        f.write("\t\tend loop;\n")
        f.write("\tend process;\n")
        ...
        f.write("\tready_gen : process\n")
        f.write("\tbegin\n")
        f.write("\t\twait until running = '1';\n")
        f.write("\t\twait until rising_edge(clock);\n")
        f.write("\t\tGET_FIFO_0_valid <= '0';\n")
        f.write("\t\tloop\n")
        f.write("\t\twait for 1000 ns;\n")
        f.write("\t\t\twait until rising_edge(clock);\n")
        f.write("\t\t\tGET_FIFO_0_valid <= '1';\n")
        f.write("\t\t\twait until GET_FIFO_0_adv = '1';\n")
        f.write("\t\t\twait until rising_edge(clock);\n")
        f.write("\t\t\tGET_FIFO_0_valid <= '0';\n")
        f.write("\t\tend loop;\n")
        f.write("\tend process;\n")
...
def test_layer(input_neurons, output_neurons, number_tests, test_time):
    parameters = generate_parameters(input_neurons, output_neurons)

    # Generate ROMs
    rom_A = []
    rom_A_width = tc_utils.unsigned.width(input_neurons)

    for output in range(output_neurons):
        # Add weights for this output neuron
        for weight in range(input_neurons):
            rom_A.append(parameters[output]["weights"][weight])
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", input_neurons, output_neurons, use_BRAMs=False, pregened_memfiles=["ROM_A"] )
...
if __name__ == "__main__":
    ...
    test_layer(input_neurons, output_neurons, number_tests, test_time)
