...
def test_layer(input_neurons, output_neurons, number_tests, test_time):
    ...
    rom_B = []
    rom_B_width = tc_utils.unsigned.width(input_neurons)
    ...
    for output in range(output_neurons):
        ...
        rom_B.append(parameters[output]["threashold"])
    ...
    with open(".\\verifying\\varifying_ROM_B.mem", "w") as f:
        for value in rom_B:
            f.write(tc_utils.unsigned.encode(value, rom_B_width))
            f.write("\n")


    # Generate program files
    generate_layer.generate_layer(".\\verifying", "varifying", input_neurons, output_neurons, use_BRAMs=False, pregened_memfiles=["ROM_A", "ROM_B", ] )
