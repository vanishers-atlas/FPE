...
def test_layer(data_rows, data_cols, data_depth, num_kernals, number_tests, test_time):
    ...
    rom_B = []
    rom_B_width = tc_utils.unsigned.width(9 * data_depth)

    # Haddle kernal region
    ...
    # Pad to pot
    while math.log(len(rom_A),2) % 1 != 0:
        rom_A.append(0)
    ...
    # Happen parameters region
    for r in range(data_rows):
        for c in range(data_cols):
            for k in range(num_kernals):
                rom_B.append(parameters[r][c][k]["threashold"])
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", data_cols, data_rows, data_depth, num_kernals, use_BRAMs=False, pregened_memfiles=["ROM_A", "ROM_B", ] )
