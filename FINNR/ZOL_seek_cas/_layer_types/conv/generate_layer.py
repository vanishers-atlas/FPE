...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals):
	...
	fpea += "COM col_ZOL : ZOL_cascade(\>\n"
	fpea += "overwrites : (data_cols),\n"
	fpea += "seekable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "COM col_plus_2_ZOL : ZOL_cascade(\>\n"
	fpea += "overwrites : (data_cols + 2),\n"
	fpea += "seekable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "COM depth_col_ZOL : ZOL_cascade(\>\n"
	...
	fpea += "COM depth_ZOL : ZOL_cascade(\>\n"
	fpea += "overwrites : (data_depth),\n"
	fpea += "seekable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "COM depth_minus_1_ZOL : ZOL_cascade(\>\n"
	fpea += "overwrites : (data_depth - 1),\n"
	fpea += "seekable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "COM depth_times_3_ZOL : ZOL_cascade(\>\n"
	...
	fpea += "// Declare seekable ZOLs\n"
	fpea += "COM kernal_ZOL : ZOL_cascade(\>\n"
...
def generate_layer_parameters_and_generics(path, layer_name, data_cols, data_rows, data_depth, num_kernals, use_BRAMs):
	...
	col_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(col_ZOL_overwrites) / 5)

	col_plus_2_ZOL_overwrites = data_cols + 2 - 1
	col_plus_2_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(col_plus_2_ZOL_overwrites) / 5)

	depth_ZOL_overwrites = data_depth - 1
	depth_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(depth_ZOL_overwrites) / 5)

	depth_col_ZOL_overwrites = data_depth * data_cols - 1
	depth_col_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(depth_col_ZOL_overwrites) / 5)

	depth_minus_1_ZOL_overwrites = data_depth - 1 - 1
	depth_minus_1_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(depth_minus_1_ZOL_overwrites) / 5)

	depth_times_3_ZOL_overwrites =  data_depth * 3 - 1
	depth_times_3_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(depth_times_3_ZOL_overwrites) / 5)

	kernal_ZOL_overwrites = num_kernals- 1
	kernal_ZOL_overwrites_width = 5 * math.ceil(tc_utils.unsigned.width(kernal_ZOL_overwrites) / 5)
