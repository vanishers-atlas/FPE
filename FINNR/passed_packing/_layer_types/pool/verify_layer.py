...
def generate_testbrench_file(input_data, output_data, packing_factor):
    with open(".\\verifying\\testbench.vhd", "w") as f:
        ...
        f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(%i downto 0);\n"%(
                (len(input_data) * len(input_data[0]) * len(input_data[0][0]) * len(input_data[0][0][0]) / packing_factor) - 1,
                packing_factor - 1,
            )
        )
        ...
        for t in sorted(input_data.keys()):
            for r in sorted(input_data[t].keys()):
                for c in sorted(input_data[t][r].keys()):
                    for word in packer(input_data[t][r][c], packing_factor):
                        f.write("\"%s\","%(word, ) )
                    f.write("\t")
                f.write("\n\t\t\t")
            f.write("\n\t\t\t")
        ...
        f.write("\t\tvariable  data_index : integer := 0;\n")
        f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(%i downto 0);\n"%(
                (len(output_data) * len(output_data[0]) * len(output_data[0][0]) * len(output_data[0][0][0]) / packing_factor) - 1,
                packing_factor - 1,
            )
        )
        f.write("\t\tconstant test_data : data_array :=\n")
        ...
        for t in sorted(output_data.keys()):
            for r in sorted(output_data[t].keys()):
                for c in sorted(output_data[t][r].keys()):
                    for word in packer(output_data[t][r][c], packing_factor):
                        f.write("\"%s\","%(word, ) )
                    f.write("\t")
                f.write("\n\t\t\t")
            f.write("\n\t\t\t")
...
def test_layer(data_rows, data_cols, data_depth, packing_factor, number_tests, test_time):

    # Generate program files
    generate_layer.generate_layer(".\\verifying", "varifying", data_rows, data_cols, data_depth, packing_factor, use_BRAMs=False, pregened_memfiles=[ ] )

    # Generate testbench
    test_input = generate_data_input(number_tests, data_rows, data_cols, data_depth)
    test_output = generate_data_output(test_input)

    generate_testbrench_file(test_input, test_output, packing_factor)
...
if __name__ == "__main__":
    ...
    print("verifying packing_factor = data_depth")
    test_layer(data_rows, data_cols, data_depth, data_depth, number_tests, test_time)
    print()

    print("verifying packing_factor = 1/2 data_depth")
    test_layer(data_rows, data_cols, data_depth, int(data_depth/2), number_tests, test_time)
    print()

    print("verifying packing_factor factor of data_depth != data_depth/2")
    test_layer(data_rows, data_cols, data_depth, int(data_depth/4), number_tests, test_time)
    print()
