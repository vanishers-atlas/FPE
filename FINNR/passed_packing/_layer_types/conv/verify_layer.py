...
def generate_testbrench_file(input_data, output_data, input_packing, output_packing):
    with open(".\\verifying\\testbench.vhd", "w") as f:
        ...
        f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(%i downto 0);\n"%(
                (len(input_data) * len(input_data[0]) * len(input_data[0][0]) * len(input_data[0][0][0]) / input_packing) - 1,
                input_packing - 1,
            )
        )
        ...
        for t in sorted(input_data.keys()):
            for r in sorted(input_data[t].keys()):
                for c in sorted(input_data[t][r].keys()):
                    for word in packer(input_data[t][r][c], input_packing):
                        f.write("\"%s\", "%(word, ) )
                    f.write("\t")
                f.write("\n\t\t\t")
            f.write("\n\t\t\t")
        f.seek(f.tell() - 13) # - 12: 2 \n, 2 \r (added as part of \n), 7 \t, 1 ",", 1 " "
        f.write("\n\t\t);\n")
        ...
        f.write("\t\tvariable  data_index : integer := 0;\n")
        f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(%i downto 0);\n"%(
                (len(output_data) * len(output_data[0]) * len(output_data[0][0]) * len(output_data[0][0][0]) / output_packing) - 1,
                output_packing - 1,
            )
        )
        f.write("\t\tconstant test_data : data_array :=\n")
        ...
        for t in sorted(output_data.keys()):
            for r in sorted(output_data[t].keys()):
                for c in sorted(output_data[t][r].keys()):
                    for word in packer(output_data[t][r][c], output_packing):
                        f.write("\"%s\", "%(word, ) )
                    f.write("\t")
                f.write("\n\t\t\t")
            f.write("\n\t\t\t")
        f.seek(f.tell() - 13) # - 10: 2 \n, 2 \r (added as part of \n), 7 \t, 1 ",", 1 " "
        f.write("\n\t\t);\n")
...
def test_layer(data_rows, data_cols, data_depth, num_kernals, input_packing, output_packing, number_tests, test_time):
    # Generate kernals
    ...
    # Generate ROMs01
    rom_A = []
    assert input_packing <= data_depth
    assert data_depth % input_packing == 0
    num_padding = int(data_depth/input_packing)

    popcount_width = tc_utils.unsigned.width(9 * data_depth)
    rom_A_width = max(popcount_width, input_packing)

    # Handle kernal region
    for k in range(num_kernals):
        for c in [-1, 0, 1]:
            for p in range(0, num_padding):
                k0 = kernals[k][-1][c][input_packing*p]
                k1 = kernals[k][ 0][c][input_packing*p]
                k2 = kernals[k][ 1][c][input_packing*p]
                for b in range(1, input_packing):
                    k0 *= 2
                    k0 += kernals[k][-1][c][input_packing*p + b]
                    k1 *= 2
                    k1 += kernals[k][ 0][c][input_packing*p + b]
                    k2 *= 2
                    k2 += kernals[k][ 1][c][input_packing*p + b]
                rom_A.append(k0)
                rom_A.append(k1)
                rom_A.append(k2)
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", data_cols, data_rows, data_depth, num_kernals, input_packing, output_packing, use_BRAMs=False, pregened_memfiles=["ROM_A", ] )
    ...
    generate_testbrench_file(test_input, test_output, input_packing, output_packing)
...
if __name__ == "__main__":
    ...
    print("verifying input_packing factor = data_depth, output_packing factor = num_kernals,")
    test_layer(data_rows, data_cols, data_depth, num_kernals, data_depth, num_kernals, number_tests, test_time)
    print()

    print("verifying input_packing factor = factor of data_depth, output_packing factor = factor of num_kernals,")
    test_layer(data_rows, data_cols, data_depth, num_kernals, int(data_depth/3), int(num_kernals/2), number_tests, test_time)
    print()

    print("verifying input_packing factor = data_depth, output_packing factor = factor of num_kernals, output_packing < input_packing")
    test_layer(data_rows, data_cols, data_depth, num_kernals, data_depth, int(num_kernals/4), number_tests, test_time)
    print()
