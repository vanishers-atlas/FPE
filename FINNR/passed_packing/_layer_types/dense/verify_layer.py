...
def generate_parameters(input_neurons, output_neurons):
    ...
    for output in range(output_neurons):
        ...
        # compute threashold
        parameters[output]["threashold"] = random.randrange(input_neurons + 1)

        # Compute gamma sign
        parameters[output]["gamma"] = random.randrange(2)
...
def generate_data_output(data_input, parameters):
    ...
    for test in range(len(data_input)):
        ...
        for output in range(output_neurons):
            ...
            # Perfrom theashold
            gamma_sign = "+" if parameters[output]["gamma"] == 1 else "-"
            diff_sign =  "+" if acc > parameters[output]["threashold"] else "-"

            if gamma_sign == diff_sign:
                test_output[test][output] = 1
            else:
                test_output[test][output] = 0
...
def generate_testbrench_file(input_data, output_data, input_packing, output_packing):
    with open(".\\verifying\\testbench.vhd", "w") as f:
        ...
        f.write("\t\tvariable  data_index : integer := 0;\n")
        f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(%i downto 0);\n"%(
                (len(output_data) * len(output_data[0]) / output_packing)- 1,
                output_packing - 1,
            )
        )
        f.write("\t\tconstant test_data : data_array :=\n")
        f.write("\t\t(\n\t\t\t")
        for test in sorted(output_data.keys()):
            for word in packer(output_data[test], output_packing):
                f.write("\"%s\", "%(word, ) )
            f.write("\n\t\t\t")
        f.seek(f.tell() - 7) # - 6: 1 \n, 1 \r (added as part of \n), 3 \t, 1 ",", 1 " "
        f.write("\n\t\t);\n")
...
def test_layer(input_neurons, output_neurons, input_packing, output_packing, number_tests, test_time):
    parameters = generate_parameters(input_neurons, output_neurons)

    # Generate ROMs
    rom_A = []
    rom_A_width = max(tc_utils.unsigned.width(input_neurons), input_packing)
    ...
    for output in range(output_neurons):
        ...
        # Add threashold for this output neuron
        rom_A.append(parameters[output]["threashold"])

        # Add gamma for this output neuron
        rom_A.append(parameters[output]["gamma"])
    ...
    generate_layer.generate_layer(".\\verifying", "varifying", input_neurons, output_neurons, input_packing, output_packing, use_BRAMs=False, pregened_memfiles=["ROM_A"] )
...
if __name__ == "__main__":
    ...
    test_layer(input_neurons, output_neurons, input_neurons, output_neurons, number_tests, test_time)
    print()

    print("verifying input_packing factor = factor of input_neurons")
    test_layer(input_neurons, output_neurons, int(input_neurons/4), int(output_neurons/4), number_tests, test_time)
    print()
