...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals):
    ...
    fpea += "DEF act_reads  1 ;\n"
    fpea += "DEF kernels    2 ;\n"
    fpea += "DEF thresholds 3 ;\n"
    fpea += "DEF gammas     3 ;\n"
    fpea += "\n"

    fpea += "DEF row_0  0*(data_cols + 2)*data_depth ;\n"
    fpea += "DEF row_1  1*(data_cols + 2)*data_depth ;\n"
    fpea += "DEF row_2  2*(data_cols + 2)*data_depth ;\n"
    ...
    fpea += "{\>\n"
    fpea += "XNOR ( RAM[BAM[act_reads]<BASE:row_0>], ROMA[BAM[kernels]<FORWARD>], ACC ) ;\n"
    fpea += "AND ( ACC, 1, ACC ) ;\n"
    fpea += "ADD ( REG[popcount], ACC, REG[popcount]) ;\n"
    fpea += "XNOR ( RAM[BAM[act_reads]<BASE:row_1>], ROMA[BAM[kernels]<FORWARD>], ACC ) ;\n"
    fpea += "AND ( ACC, 1, ACC ) ;\n"
    fpea += "ADD ( REG[popcount], ACC, REG[popcount]) ;\n"
    fpea += "XNOR ( RAM[BAM[act_reads]<BASE:row_2, FORWARD>], ROMA[BAM[kernels]<FORWARD>], ACC ) ;\n"
    fpea += "AND ( ACC, 1, ACC ) ;\n"
    fpea += "ADD ( REG[popcount], ACC, REG[popcount]) ;\n"
    fpea += "\n"
    fpea += "\<}\n"
    ...
    fpea += "SEEK BAM[act_reads] (3 * data_depth)<BACKWARD>;\n"

    fpea += "\<}\n"
    fpea += "\n"

    fpea += "// Seek acc_row BAMs forward for next col\n"
    fpea += "SEEK BAM[act_reads] (data_depth)<FORWARD>;\n"

    fpea += "\<}\n"
    fpea += "\n"

    fpea += "// Seek acc_row BAMs forward for next row\n"
    fpea += "SEEK BAM[act_reads] (2 * data_depth)<FORWARD>;\n"
