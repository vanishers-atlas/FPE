    ...
    with open(".\\verifying\\testbench.vhd", "w") as f:
        ...
        for lane in range(num_lanes):
            ...
            f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(0 downto 0);\n"%(
                    len(input_data[lane]) * len(input_data[lane][0]) - 1,
                )
            )
            f.write("\t\tconstant test_data : data_array :=\n")
            f.write("\t\t(\n\t\t\t")
            for t in sorted(input_data[lane].keys()):
                for o in sorted(input_data[lane][t].keys()):
                    f.write("\"%i\","%(input_data[lane][t][o], ) )
                f.write("\n\t\t\t")
            ...
            for t in sorted(output_data[lane].keys()):
                for o in sorted(output_data[lane][t].keys()):
                    f.write("\"%i\","%(output_data[lane][t][o], ) )
                f.write("\n\t\t\t")
            f.seek(f.tell() - 6) # - 6: 1 \n, 1 \r (added as part of \n), 3 \t, 1 ","
            f.write("\n\t\t);\n")
...
def test_layer(input_neurons, output_neurons, num_lanes, number_tests, test_time):
    ...
    with open(".\\verifying\\varifying_ROM_A.mem", "w") as f:
        for value in rom_A:
            ...
            f.write("\n")
        pregened_memfiles = {
            "LANE_%i_ROM_A"%(lane, ) : ".\\varifying_ROM_A.mem" for lane in range(num_lanes)
        }
    ...
    test_output = []
    for lane in range(num_lanes):
        test_input.append(generate_data_input(number_tests, input_neurons))
        test_output.append(generate_data_output(test_input[-1], parameters))
