...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals):
    ...
    fpea += "// Pad first row,\n"
    fpea += "MOV(0, ACC) ;\n"
    fpea += "REP (data_cols + 2)\n"
    fpea += "{\>\n"
    fpea += "REP (data_depth / 2)\n"
    fpea += "{\>\n"
    ...
    fpea += "REP (data_depth * data_cols / 2)\n"
    fpea += "{\>\n"
