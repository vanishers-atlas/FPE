...
def generation_network(network_name, input_dims, layers, use_BRAMs=True):
    ...
    network_folder = os.path.dirname(__file__) + "\\" + network_name
    try:
        os.makedirs(network_folder + "\\toolchain_files")
    except FileExistsError:
        pass

    # Generate RS FF for flagging in the network
    shutil.copyfile("SR_flip_flop.vhd", network_folder + "\\SR_flip_flop.vhd")

    # Copy edge dectector into network dict
    shutil.copyfile("edge_detector.vhd", network_folder + "\\edge_detector.vhd")
    ...
    for layer, layer_details in enumerate(layers):
        ...
        network_script += "create_bd_cell -type module -reference edge_detector layer_%i_running_edge_detector\n"%(layer, )

        if layer != 0:
            network_script += "create_bd_cell -type module -reference SR_flip_flop layer_%i_data_ready\n"%(layer, )
            network_script += "set_property -dict [list CONFIG.start_reset {true}] [get_bd_cells layer_%i_data_ready]\n"%(layer, )

        if layer != LAST_LAYER:
            network_script += "create_bd_cell -type module -reference SR_flip_flop layer_%i_space_ready\n"%(layer, )
            network_script += "set_property -dict [list CONFIG.start_set {true}] [get_bd_cells layer_%i_space_ready]\n"%(layer, )
        ...
        network_script += "CONFIG.Input_Depth {%i} "%(2*output_size_pot, )
        network_script += "CONFIG.Output_Data_Width {%i} "%(data_width, )
        network_script += "CONFIG.Output_Depth {%i} "%(2*output_size_pot, )
        network_script += "CONFIG.Reset_Pin {false} "
        network_script += "CONFIG.Reset_Type {Asynchronous_Reset} "
        network_script += "CONFIG.Use_Dout_Reset {false} "
        network_script += "CONFIG.Use_Extra_Logic {true} "
        ...
        network_script += "CONFIG.Full_Threshold_Assert_Value {%i} "%(output_size_pot - 1, )
        network_script += "CONFIG.Full_Threshold_Negate_Value {%i} "%(output_size_pot - 2, )
        network_script += "CONFIG.Empty_Threshold_Assert_Value {4} "
        network_script += "CONFIG.Empty_Threshold_Negate_Value {5}] "
        ...
        network_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_running_edge_detector/clock]\n"%(layer, )
        network_script += "connect_bd_net [get_bd_pins layer_%i/running] [get_bd_pins layer_%i_running_edge_detector/probe]\n"%(layer, layer, )
        ...
        if layer == 0:
            network_script += "connect_bd_net [get_bd_pins fifo_0/prog_full] [get_bd_pins layer_0_kickoff_concat/In0]\n"
        else:
            network_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_data_ready/clock]\n"%(layer, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/found_rising_edge] [get_bd_pins layer_%i_data_ready/R]\n"%(layer, layer, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/found_falling_edge] [get_bd_pins layer_%i_data_ready/S]\n"%(layer - 1, layer, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_data_ready/Q] [get_bd_pins layer_%i_kickoff_concat/In0]\n"%(layer, layer, )

        if layer != LAST_LAYER:
            network_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_space_ready/clock]\n"%(layer, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/found_rising_edge] [get_bd_pins layer_%i_space_ready/R]\n"%(layer, layer, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_space_ready/Q] [get_bd_pins layer_%i_kickoff_concat/In1]\n"%(layer, layer, )
        if layer != 0:
            network_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/found_rising_edge] [get_bd_pins layer_%i_space_ready/S]\n"%(layer, layer - 1, )
    ...
    network_script += "set_property -dict [list CONFIG.Programmable_Empty_Type {Single_Programmable_Empty_Threshold_Constant} "
    network_script += "CONFIG.Empty_Threshold_Assert_Value {%i} "%(output_size, )
    network_script += "CONFIG.Empty_Threshold_Negate_Value {%i}] [get_bd_cells fifo_%i]\n"%(output_size + 1, LAST_LAYER + 1, )
