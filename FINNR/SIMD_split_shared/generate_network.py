...
def generation_network(network_name, input_dims, layers, use_BRAMs=True):
    ...
    for layer, layer_details in enumerate(layers):
        ...
        # Generate piso_sequencer's toolchain files
        if layer_details["lanes"] > 1:
            piso_interface, piso_name = HDL_generator.generate_module(
                module_select = "network.piso_sequencer_"+layer_details["piso_type"],
                config = { "inputs" : layer_details["lanes"], },
                module_name = None,
                HDL_output_path=network_folder + "\\toolchain_files",
                concat_naming = False,
                force_generation = True
            )

            network_script += "create_bd_cell -type module -reference %s layer_%i_piso\n\n"%(piso_name, layer, )
            network_script += "set_property -dict [list CONFIG.data_width {%i}] [get_bd_cells layer_%i_piso]\n"%(data_width, layer, )
        ...
        if layer_details["lanes"] > 1:
            for lane in range(layer_details["lanes"]):
                 network_script += "connect_bd_net [get_bd_pins layer_%i/LANE_%i_PUT_FIFO_0_data] [get_bd_pins layer_%i_piso/data_in_%i]\n"%(layer, lane, layer, lane, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_piso/data_out] [get_bd_pins fifo_%i/din]\n"%(layer, output_FIFO, )

            network_script += "connect_bd_net [get_bd_pins layer_%i/LANE_0_PUT_FIFO_0_write] [get_bd_pins layer_%i_piso/inputs_write]\n"%(layer, layer, )
            network_script += "connect_bd_net [get_bd_pins layer_%i_piso/output_write] [get_bd_pins fifo_%i/wr_en]\n"%(layer, output_FIFO, )
        else:
            network_script += "connect_bd_net [get_bd_pins layer_%i/LANE_0_PUT_FIFO_0_data] [get_bd_pins fifo_%i/din]\n"%(layer, output_FIFO, )
            network_script += "connect_bd_net [get_bd_pins layer_%i/LANE_0_PUT_FIFO_0_write] [get_bd_pins fifo_%i/wr_en]\n"%(layer, output_FIFO, )

        if layer_details["type"] == "pool":
            network_script += "connect_bd_net [get_bd_pins fifo_%i/dout] [get_bd_pins layer_%i/LANE_0_GET_FIFO_0_data]\n"%(input_FIFO, layer, )
        else:
        ...
        if layer_details["lanes"] > 1:
            network_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_piso/clock]\n"%(layer, )
