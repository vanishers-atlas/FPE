...
def realise_template(output_path, output_name, input_neurons, output_neurons, use_BRAMs, romA_mif=None, romB_mif=None):
    ...
    BNN_utils.realise_template(
        ...
        path_preface + "\\dense_dynamic",
        {
            ...
            "ROMB_MIF" : romB_mif if romB_mif else ".\\blankmem_1x%i.mem"%(output_neurons, )
        }
    )
...
if __name__ == '__main__':
    output_path = ".\\dense_dynamic"
    output_name = "dense_test"
    ...
    romA_file = "ROMA.mif"
    romB_file = "ROMB.mif"
    realise_template(output_path, output_name, input_neurons, output_neurons, use_BRAMs=False, romA_mif = "..\\" + romA_file, romB_mif = "..\\" + romB_file)
    ...
    test_output = BNN_utils.dense_outputs(number_tests, input_neurons, test_input, output_neurons, parameters)

    # Write ROM file+
    with open(output_path + "\\" + romA_file, "w") as f:
        for o in range(output_neurons):
            for i in range(input_neurons):
                f.write(tc_utils.unsigned.encode(parameters[o]["weights"][i], 1))
                f.write("\n")
            f.write(tc_utils.unsigned.encode(parameters[o]["gamma"], 1))
            f.write("\n")

    rom_width = tc_utils.unsigned.width(input_neurons)
    with open(output_path + "\\" + romB_file, "w") as f:
        for o in range(output_neurons):
            f.write(tc_utils.unsigned.encode(parameters[o]["threashold"], rom_width))
            f.write("\n")
    ...
    with open(output_path + "\\testbench.vhd", "w") as f:
        ...
        f.write("\t\tconstant test_data : data_array :=\n")
        f.write("\t\t(\n")
        f.write(BNN_utils.format_1d_data(number_tests, output_neurons, test_output, line_start="\t\t\t", encode_width=1))
        f.write("\n\t\t);\n")
