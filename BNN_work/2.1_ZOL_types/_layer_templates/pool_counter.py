...
def realise_template(output_path, output_name, data_rows, data_cols, data_depth, use_BRAMs):
    assert type(output_path) == str
    assert type(output_name) == str

    assert type(data_rows) == int and math.log(data_rows,2) % 1 == 0, "data_rows must be a power of 2"
    assert type(data_cols) == int and math.log(data_cols,2) % 1 == 0, "data_cols must be a power of 2"
    assert type(data_depth) == int and math.log(data_depth,2) % 1 == 0, "data_depth must be a power of 2"
    ...
    return (int(data_rows/2), int(data_cols/2), data_depth)

# When runing as script verity layer
if __name__ == '__main__':
    output_path = ".\\pool_counter"
    output_name = "pooling_test"
    ...
    number_tests, data_rows, data_cols, data_depth = 12, 8, 4, 8

    # Create FPEA, parameter, and generics files
    realise_template(output_path, output_name, data_rows, data_cols, data_depth, use_BRAMs=False)

    # Generate test data
    test_input = BNN_utils.generate_2d_data(number_tests, data_rows, data_cols, data_depth)
    test_output = BNN_utils.pooling_outputs(number_tests, data_rows, data_cols, data_depth, test_input)
    ...
    with open(output_path + "\\testbench.vhd", "w") as f:
        ...
        f.write("\t\tvariable  data_index : integer := 0;\n")
        f.write("\t\ttype  data_array is array (0 to %i) of std_logic_vector(0 downto 0);\n"%(
                (number_tests * data_rows * data_cols * data_depth / 4) - 1
            )
        )
        f.write("\t\tconstant test_data : data_array :=\n")
        f.write("\t\t(\n")
        f.write(BNN_utils.format_2d_data(number_tests, int(data_rows/2), int(data_cols/2), data_depth, test_output, line_start="\t\t\t", encode_width=1))
        f.write("\n\t\t);\n")
