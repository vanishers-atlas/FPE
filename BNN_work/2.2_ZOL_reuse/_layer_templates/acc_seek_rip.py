...
def realise_template(output_path, output_name, input_neurons, output_neurons, use_BRAMs, rom_mif=None):
	...
	input_ZOL_overwrites = tc_utils.biased_tally.encode(input_ZOL_overwrites, input_ZOL_overwrites_width, 1, 31)

	path_preface = "\\".join(__file__.split("\\")[:-1])
	BNN_utils.realise_template(
		output_path,
		output_name,
		path_preface + "\\acc_seek_rip",
		{
			"INPUT_NEURONS" : input_neurons,
			"OUTPUT_NEURONS" : output_neurons,
			"POPCOUNT_WIDTH" : popcount_width,
			"INPUT_ZOL_OVERWRITES" : input_ZOL_overwrites,
