...
def generate_layer_fpea(path, layer_name, data_cols, data_rows, data_depth, num_kernals):
	...
	fpea += "// Declare seekable settable ZOLs\n"
	fpea += "COM ZOL_A : ZOL_counter(\>\n"
	fpea += "overwrites : (data_cols + 1),\n"
	fpea += "seekable : true,\n"
	fpea += "settable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "COM ZOL_B : ZOL_counter(\>\n"
	if data_cols > data_depth:
		fpea += "overwrites : (data_cols - 1),\n"
	else:
	...
	fpea += "COM ZOL_C : ZOL_counter(\>\n"
	fpea += "overwrites : (num_kernals - 1),\n"
	fpea += "seekable : true,\n"
	fpea += "settable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "COM ZOL_D : ZOL_counter(\>\n"
	fpea += "overwrites : (data_depth * data_cols - 1),\n"
	fpea += "seekable : true,\n"
	fpea += "settable : true\n"
	fpea += "\<);\n"
	fpea += "\n"

	fpea += "// Setup ZOLs for first and second row loops\n"
	fpea += "ZOL_A.SEEK(first_row_pad_outer);\n"
	fpea += "ZOL_A.SET(data_cols + 2 - 1);\n"
	fpea += "ZOL_B.SEEK(first_row_pad_inter);\n"
	fpea += "ZOL_B.SET(data_depth - 1);\n"
	fpea += "ZOL_C.SEEK(second_row_front_pad);\n"
	fpea += "ZOL_C.SET(data_depth - 1 - 1);\n"
	fpea += "ZOL_D.SEEK(second_row_data_read);\n"
	fpea += "ZOL_D.SET(data_depth * data_cols - 1);\n"
	...
	fpea += "{\>\n"
	fpea += "LOOP first_row_pad_inter : // data_depth\n"
	fpea += "{\>\n"
	...
	fpea += "// Set up ZOL_C  and ZOL_A for  next unnested loops,\n"
	fpea += "ZOL_C.SEEK(second_row_back_pad);\n"
	fpea += "ZOL_A.SEEK(main_silding_window_row);\n"
	fpea += "ZOL_A.SET(data_rows - 1 - 1);\n"
	...
	fpea += "// Setup ZOL_B for main loops\n"
	fpea += "ZOL_B.SEEK(main_silding_window_col);\n"
	fpea += "ZOL_B.SET(data_cols - 1);\n"

	fpea += "// Process all but last row\n"
	fpea += "LOOP main_silding_window_row : // data_rows - 1\n"
	fpea += "{\>\n"

	fpea += "// Setup ZOLs for first and second row loops\n"
	fpea += "ZOL_C.SEEK(main_row_front_pad);\n"
	fpea += "ZOL_C.SET(data_depth - 1 - 1);\n"
	fpea += "ZOL_D.SEEK(main_row_data_read);\n"
	fpea += "ZOL_D.SET(data_depth * data_cols - 1);\n"
	...
	fpea += "// Seek ZOL_C to pad the  end of the row\n"
	fpea += "ZOL_C.SEEK(main_row_back_pad);\n"
	...
	fpea += "// Setup ZOL_C and ZOL_D for  popcount loops\n"
	fpea += "ZOL_C.SEEK(main_kernal_loop);\n"
	fpea += "ZOL_C.SET(num_kernals - 1);\n"
	fpea += "ZOL_D.SEEK(main_popcount_compute);\n"
	fpea += "ZOL_D.SET(3 * data_depth - 1);\n"

	fpea += "// Slide window across rows\n"
	fpea += "LOOP main_silding_window_col : // data_cols\n"
	fpea += "{\>\n"
	...
	fpea += "LOOP main_kernal_loop: // num_kernals\n"
	fpea += "{\>\n"
	...
	fpea += "LOOP main_popcount_compute : // 3 * data_depth \n"
	fpea += "{\>\n"
	...
	fpea += "// Setup ZOLs for last row loops\n"
	fpea += "ZOL_A.SEEK(last_row_pad_outer);\n"
	fpea += "ZOL_A.SET(data_cols + 2 - 1);\n"
	fpea += "ZOL_B.SEEK(last_row_pad_inter);\n"
	fpea += "ZOL_B.SET(data_depth - 1);\n"
	fpea += "ZOL_C.SEEK(last_kernal_loop);\n"
	fpea += "ZOL_C.SET(num_kernals - 1);\n"
	fpea += "NOP;\n"
	...
	fpea += "{\>\n"
	fpea += "LOOP last_row_pad_inter : // data_depth\n"
	fpea += "{\>\n"
	fpea += "NOT(ACC, RAM[BAM[act_write]<FORWARD>] ) ;\n"
	fpea += "\<}\n"
	fpea += "NOT(ACC, ACC) ;\n"
	...
	fpea += "// Setup ZOLs for last row loops\n"
	fpea += "ZOL_B.SEEK(last_silding_window_col);\n"
	fpea += "ZOL_B.SET(data_cols - 1);\n"
	fpea += "ZOL_D.SEEK(last_popcount_compute);\n"
	fpea += "ZOL_D.SET(3 * data_depth - 1);\n"

	fpea += "// Slide window across rows\n"
	fpea += "LOOP last_silding_window_col : // data_cols\n"
	fpea += "{\>\n"
	...
	fpea += "// Process each kernel\n"
	fpea += "LOOP last_kernal_loop : // num_kernals\n"
	fpea += "{\>\n"
	...
	fpea += "// Compute popcount\n"
	fpea += "// * 3 for each col of the kernel\n"
	fpea += "LOOP last_popcount_compute : // 3 * data_depth \n"
	fpea += "{\>\n"
	fpea += "XNOR ( RAM[BAM[act_row_0]<FORWARD>], ROMA[BAM[kernels]<FORWARD>], ACC ) ;\n"
	fpea += "AND ( ACC, 1, ACC ) ;\n"
	fpea += "ADD ( REG[popcount], ACC, REG[popcount]) ;\n"
