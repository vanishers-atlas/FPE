...
def realise_template(output_path, output_name, data_rows, data_cols, data_depth, num_kernals, use_BRAMs, rom_mif=None):
    ...
    BNN_utils.realise_template(
        ...
        path_preface + "\\conv",
        {
            ...
            "ROM_MIF" : rom_mif if rom_mif else ".\\blankmem_1x%i.mem"%(ROM_depth, )
        }
    )
...
if __name__ == '__main__':
    ...
    rom_file = "ROM.mif"
    realise_template(output_path, output_name, data_rows, data_cols, data_depth, num_kernals, use_BRAMs=False, rom_mif = "..\\" + rom_file)
        ...
        for k in range(num_kernals):
            for x in [-1, 0, 1]:
                for d in range(data_depth):
                    for y in [-1, 0, 1]:
                        f.write(tc_utils.unsigned.encode(kernals[k]["weights"][x][y][d], rom_width))
                        f.write("\n")
            f.write(tc_utils.unsigned.encode(kernals[k]["threashold"], rom_width))
            f.write("\n")
            f.write(tc_utils.unsigned.encode(kernals[k]["gamma"], rom_width))
            f.write("\n")
