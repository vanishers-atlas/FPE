...
def generate_tcl_script(network_name, network_folder, input_dims, network_structure):
    ...
    for layer, layer_details in enumerate(network_structure):
        ...
        tcl_script += "CONFIG.Programmable_empty_Type {Single_Programmable_Empty_Threshold_Constant} "
        tcl_script += "CONFIG.Empty_Threshold_Assert_Value {%i} "%(frame_size - 1, )
        tcl_script += "CONFIG.Empty_Threshold_Negate_Value {%i} "%(frame_size - 2, )
    ...
    tcl_script += "CONFIG.Write_Data_Count_Width {10} "
    tcl_script += "CONFIG.Read_Data_Count_Width {10} "
    tcl_script += "CONFIG.Programmable_Full_Type {No_Programmable_Full_Threshold} "
    tcl_script += "CONFIG.Full_Threshold_Assert_Value {%i} "%(output_depth - 1, )
    tcl_script += "CONFIG.Full_Threshold_Negate_Value {%i} "%(output_depth - 2, )
    tcl_script += "CONFIG.Programmable_empty_Type {Single_Programmable_Empty_Threshold_Constant} "
    tcl_script += "CONFIG.Empty_Threshold_Assert_Value {%i} "%(output_depth - 1, )
    tcl_script += "CONFIG.Empty_Threshold_Negate_Value {%i} "%(output_depth - 2, )
    tcl_script += "] [get_bd_cells fifo_%i]\n\n"%(OUTPUT_FIFO, )
    ...
    for layer in range(len(network_structure)):
        ...
        tcl_script += "connect_bd_net [get_bd_pins fifo_%i/prog_full] [get_bd_pins layer_%i_kickoff_concat/In0]\n"%(layer, layer )
        tcl_script += "connect_bd_net [get_bd_pins fifo_%i/prog_empty] [get_bd_pins layer_%i_kickoff_concat/In1]\n"%(layer + 1, layer )
...
if __name__ == "__main__":
    ...
    network_name = "base_dev_FPS_fifo"
