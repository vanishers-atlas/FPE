...
def generate_tcl_script(network_name, network_folder, input_dims, network_structure):
    ...
    for layer, layer_details in enumerate(network_structure):
        ...
        fifo_depth = 2**math.ceil(math.log(2 * frame_size, 2))
    ...
    HDL_generator.generate_module("network.edge_detector", {}, module_name = "edge_detector", HDL_output_path=fullpath, concat_naming = False, force_generation = True)
    HDL_generator.generate_module("network.SR_FF", {}, module_name = "SR_FF", HDL_output_path=fullpath, concat_naming = False, force_generation = True)

    tcl_script +=  "read_vhdl -library work %s/edge_detector.vhd;\n"%(fullpath.replace("\\", "/"), )
    tcl_script +=  "read_vhdl -library work %s/SR_FF.vhd;\n"%(fullpath.replace("\\", "/"), )

    # Setup edge detectors
    for layer in range(len(network_structure)):
        tcl_script += "create_bd_cell -type module -reference edge_detector layer_%i_running_edge_detector\n"%(layer, )

        tcl_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_running_edge_detector/clock]\n"%(layer, )
        tcl_script += "connect_bd_net [get_bd_pins layer_%i/running] [get_bd_pins layer_%i_running_edge_detector/detector]\n\n"%(layer, layer )

    # Setup flag logic
    for layer in range(len(network_structure)):
        if layer != 0:
            # Setup data frame flag SR_FF
            tcl_script += "create_bd_cell -type module -reference SR_FF layer_%i_data_ready\n"%(layer, )
            tcl_script += "set_property -dict [list CONFIG.start_reset {true}] [get_bd_cells layer_%i_data_ready]\n"%(layer, )

            tcl_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_data_ready/clock]\n"%(layer, )
            tcl_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/falling_edge_detected] [get_bd_pins layer_%i_data_ready/S]\n"%(layer - 1, layer )
            tcl_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/rising_edge_detected] [get_bd_pins layer_%i_data_ready/R]\n"%(layer, layer )


        if layer != LAST_LAYER:
            # Setup data frame flag SR_FF
            tcl_script += "create_bd_cell -type module -reference SR_FF layer_%i_space_ready\n"%(layer, )
            tcl_script += "set_property -dict [list CONFIG.start_set {true}] [get_bd_cells layer_%i_space_ready]\n"%(layer, )

            tcl_script += "connect_bd_net [get_bd_ports clock] [get_bd_pins layer_%i_space_ready/clock]\n"%(layer, )
            tcl_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/rising_edge_detected] [get_bd_pins layer_%i_space_ready/S]\n"%(layer + 1, layer )
            tcl_script += "connect_bd_net [get_bd_pins layer_%i_running_edge_detector/rising_edge_detected] [get_bd_pins layer_%i_space_ready/R]\n"%(layer, layer )

    # Setup kickout anding logic
    for layer in range(len(network_structure)):
        tcl_script += "create_bd_cell -type ip -vlnv xilinx.com:ip:xlconcat:2.1 layer_%i_kickoff_concat\n"%(layer, )
        tcl_script += "set_property -dict [list CONFIG.NUM_PORTS {2}] [get_bd_cells layer_%i_kickoff_concat]\n"%(layer, )

        tcl_script += "create_bd_cell -type ip -vlnv xilinx.com:ip:util_reduced_logic:2.0 layer_%i_kickoff_reduce\n"%(layer, )
        tcl_script += "set_property -dict [list CONFIG.C_SIZE {2}] [get_bd_cells layer_%i_kickoff_reduce]\n"%(layer, )

        if layer == 0:
            tcl_script += "create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 data_always_ready\n"
            tcl_script += "connect_bd_net [get_bd_pins data_always_ready/dout] [get_bd_pins layer_%i_kickoff_concat/In0]\n"%(layer, )
        else:
            tcl_script += "connect_bd_net [get_bd_pins layer_%i_data_ready/Q] [get_bd_pins layer_%i_kickoff_concat/In0]\n"%(layer, layer )
        if layer == LAST_LAYER:
            tcl_script += "create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 space_always_ready\n"
            tcl_script += "connect_bd_net [get_bd_pins space_always_ready/dout] [get_bd_pins layer_%i_kickoff_concat/In1]\n"%(layer, )
        else:
            tcl_script += "connect_bd_net [get_bd_pins layer_%i_space_ready/Q] [get_bd_pins layer_%i_kickoff_concat/In1]\n"%(layer, layer )
...
if __name__ == "__main__":
    ...
    network_name = "base_dev_FPS_flag"
