...
entity axis_test_ip_v2_0_S00_AXIS is
	generic (
		...
		NUMBER_OF_INPUT_WORDS	: integer	:= 10
	);
	port (
...
end axis_test_ip_v2_0_S00_AXIS;

architecture arch_imp of axis_test_ip_v2_0_S00_AXIS is
	...
	-- constant NUMBER_OF_INPUT_WORDS  : integer := 10;
...
begin
    ...
    -- axis_tready <= '1' when (write_pointer <= NUMBER_OF_INPUT_WORDS-1) else '0';
...
--				 write_pointer <= 0;
	...
	-- FIFO Implementation
     ...
     FIFO_GEN: for byte_index in 0 to (NUMBER_OF_INPUT_WORDS-1) generate
	 ...
	 begin   
	  ...
	  begin
	    if (rising_edge (S_AXIS_ACLK)) then
	      if (fifo_wren = '1') then
	        -- stream_data_fifo(write_pointer-1) <= S_AXIS_TDATA((byte_index*8+7) downto (byte_index*8));
