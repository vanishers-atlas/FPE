...
def run_generation(input_neurons, output_neurons, number_tests):
    ...
    weights, threasholds = generate_neuron_parameters(input_neurons, output_neurons)

    # Save Neuron parameters in a mem filw for t3esting
    generate_ROM_mem_file(weights, threasholds)

    # Generate test input
    test_inputs = generate_test_input(input_neurons, number_tests)

    # Compute output for each test
    test_outputs = generate_test_output(test_inputs, weights, threasholds)
...
def generate_neuron_parameters(input_neurons, output_neurons):
    ...
    for _ in range(output_neurons):
        # + 2 as @
        # threashold can take any value of the popcount (0 to number_input_neurons + 1)
        # randrange returns 0 <= and < end, so + 1 tp include number_input_neurons + 1
        threasholds.append(random.randrange(input_neurons + 2))
    ...
    return weights, threasholds

def generate_ROM_mem_file(weights, threasholds):
    ROM_width = tc_utils.unsigned.width(len(weights[0]))
    with open("test_ROM.mem", "w") as f:
        for (neuron_weights, threashold) in zip(weights, threasholds):
            for weight in neuron_weights:
                f.write(tc_utils.unsigned.encode(weight, ROM_width))
                f.write("\n")
            f.write(tc_utils.unsigned.encode(threashold, ROM_width))
            f.write("\n")
...
def generate_test_output(inputs, weights, threasholds):
    test_output = []
    for input in inputs:
        test_output.append([])
        for (neuron_weights, neuron_threashold) in zip(weights, threasholds):
            products = [ w ^ i
            ...
            if popcount > neuron_threashold:
                test_output[-1].append(1)
            else:
                test_output[-1].append(0)
