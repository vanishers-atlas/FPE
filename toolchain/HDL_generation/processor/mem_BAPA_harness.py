...
def preprocess_config(config_in):
    ...
    assert len(config_in["writes"]) > 0, "writes cab't be empty"
    config_out["writes"] = []
...
def gen_reads():
    ...
    block_addr_bits = int(math.log(CONFIG["num_blocks"], 2))

    for read, details in enumerate(CONFIG["reads"]):
        lg_min = int(math.log(details["min"], 2))
        lg_max = int(math.log(details["max"], 2))

        # Handle addr ports
        INTERFACE["ports"]["read_%i_addr"%(read, )] = {
            "type" : "std_logic_vector(block_addr_width + %i downto 0)"%(block_addr_bits - 1, ),
            "direction" : "in",
        }

        for block in range(CONFIG["num_blocks"]):
            INTERFACE["ports"]["block_%i_read_%i_addr"%(block, read, )] = {
                "type" : "std_logic_vector(block_addr_width - 1 downto 0)",
                "direction" : "out",
            }

            ARCH_BODY += "block_%i_read_%i_addr <= read_%i_addr(read_%i_addr'left downto %i);\n"%(block, read, read, read, block_addr_bits,)
        ...
        for word in range(details["max"]):
            INTERFACE["ports"]["read_%i_word_%i"%(read, word,)] = {
                "type" : "std_logic_vector(data_width - 1 downto 0)",
                "direction" : "out",
            }

            ARCH_HEAD += "signal read_%i_word_%i_reg_in : std_logic_vector(data_width - 1 downto 0);\n"%(read, word,)

            ARCH_BODY += " read_%i_word_%i_reg : entity work.%s(arch)\>\n"%(read, word, reg_name)
            ARCH_BODY += "generic map (data_width => data_width)\n"
            ARCH_BODY += "port map (\n\>"
        ...
        for block in range(CONFIG["num_blocks"]):
            INTERFACE["ports"]["block_%i_read_%i_data"%(block, read, )] = {
                "type" : "std_logic_vector(data_width - 1 downto 0)",
                "direction" : "in",
            }
        ...
        ###########################################################################################
        mux_tree_signals = { block_addr_bits : [ "block_%i_read_%i_data"%(block, read, ) for block in range(CONFIG["num_blocks"]) ], }
        ...
        for addr_bit in reversed(range(lg_min, block_addr_bits)):
            ...
            for mux_index in range(num_muxes):
                ...
                ARCH_BODY += "data_out => read_%i_mux_%i_%i_out\n"%(read, addr_bit, mux_index, )
...
def gen_writes():
    ...
    for write, details in enumerate(CONFIG["writes"]):
        lg_min = int(math.log(details["min"], 2))
        lg_max = int(math.log(details["max"], 2))
        ...
        for block in range(CONFIG["num_blocks"]):
            INTERFACE["ports"]["block_%i_write_%i_addr"%(block, write, )] = {
                "type" : "std_logic_vector(block_addr_width - 1 downto 0)",
                "direction" : "out",
            }

            ARCH_BODY += "block_%i_write_%i_addr <= write_%i_addr(write_%i_addr'left downto %i);\n"%(block, write, write, write, block_addr_bits,)
        ...
        for lg_block_size in range(lg_min, lg_max + 1):
            ...
            INTERFACE["ports"]["write_%i_enable_%i"%(write, block_size, )] = {
                "type" : "std_logic",
                "direction" : "in",
            }

            num_regions = int(CONFIG["num_blocks"]/block_size)
            ...
            if num_regions == 1:
                ...
                # Instancate demux
                ARCH_BODY += "write_%i_enable_%i_demux : entity work.%s(arch)\>\n"%(write, block_size, demux_name, )

                ARCH_BODY += "generic map (data_width => 1)\n"

                ARCH_BODY += "port map (\n\>"
                ARCH_BODY += "sel => write_%i_addr(%i downto %i),\n"%(write, lg_block_size + int(math.log(num_regions, 2)) - 1, lg_block_size, )
                ARCH_BODY += "data_in(0) => write_%i_enable_%i,\n"%(write, block_size, )

                for region in range(num_regions):
                    ARCH_HEAD += "signal write_%i_enable_%i_demux_out_%i : std_logic;\n"%(write, block_size, region, )
                    ARCH_BODY += "data_out_%i(0) => write_%i_enable_%i_demux_out_%i,\n"%(region, write, block_size, region, )
        ...
        for block in range(CONFIG["num_blocks"]):
            INTERFACE["ports"]["block_%i_write_%i_enable"%(block, write, )] = {
                "type" : "std_logic",
                "direction" : "out",
            }
            normalized_block = block/CONFIG["num_blocks"]
            terms = []
            for lg_block_size in range(lg_min, lg_max + 1):
                block_size = 2 ** lg_block_size
                index = math.floor(normalized_block*len(enable_demux_signals[block_size]))
                terms.append(enable_demux_signals[block_size][index])
            ARCH_BODY += "block_%i_write_%i_enable <= %s;\n"%(block, write, " or ".join(terms))

        # Handle data ports
        for word in range(details["max"]):
            INTERFACE["ports"]["write_%i_word_%i"%(write, word,)] = {
                "type" : "std_logic_vector(data_width - 1 downto 0)",
                "direction" : "in",
            }

        for block in range(CONFIG["num_blocks"]):
            INTERFACE["ports"]["block_%i_write_%i_data"%(block, write, )] = {
                "type" : "std_logic_vector(data_width - 1 downto 0)",
                "direction" : "out",
            }
        ...
        for  word in range(details["min"]):
            ARCH_HEAD += "signal write_%i_word_%i_muxed : std_logic_vector(data_width - 1 downto 0);\n"%(write, word, )
            ARCH_BODY += "write_%i_word_%i_muxed <=  write_%i_word_%i;\n"%(write, word, write, word, )
            mux_chain_signals.append("write_%i_word_%i_muxed"%(write, word, ))

        block_size = details["min"]
        while block_size < CONFIG["num_blocks"]:
            for index in range(block_size):
                ...
                if len(mux_chain_sel_terms) > 0:
                    ...
                    ARCH_BODY += "write_%i_word_%i_mux : entity work.%s(arch)\>\n"%(write, word, mux_name, )
                    ...
                    ARCH_BODY += "data_in_1 => write_%i_word_%i,\n"%(write, word, )
                    ARCH_BODY += "data_out => write_%i_word_%i_muxed\n"%(write, word, )
        ...
        for block, signal in zip(range(CONFIG["num_blocks"]), it.cycle(mux_chain_signals)):
            ARCH_BODY += "block_%i_write_%i_data <= %s;\n"%(block, write, signal, )
