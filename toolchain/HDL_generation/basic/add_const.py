...
def generate_HDL(config, output_path, module_name, concat_naming=False, force_generation=False):
    ...
    try:
        ...
        com_det.add_port("value_out", "std_logic_vector(%i downto 0)"%(gen_det.config["width"] - 1,), "out")

        com_det.arch_body += "value_out <= std_logic_vector(to_unsigned(to_integer(unsigned(value_in)) + const, %i));\n\n"%(gen_det.config["width"], )
