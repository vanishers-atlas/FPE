...
def preprocess_config(config_in):
    ...
    assert(config_in["outputs"] >= 1)
    config_out["sel_width"] = tc_utils.unsigned.width(config_in["outputs"] - 1)
    config_out["outputs"] = 2**config_out["sel_width"]
...
def handle_module_name(module_name, config):
    if module_name == None:
        ...
        generated_name = "demux"

        generated_name += "_%i"%(config["outputs"])
...
def generate_HDL(config, output_path, module_name, concat_naming=False, force_generation=False):
    ...
    try:
        ...
        # Add number of outputs and sel_width into interface so other files can access them
        INTERFACE["number_outputs"] = CONFIG["outputs"]
